...
import { Users, Plus, AlertTriangle, CheckCircle2, X, Shield, Edit2, Trash2, Calendar } from "lucide-react";
...
const emptyForm = { employee_id: "", name: "", email: "", base_salary: "", social_security: false, religion: "" };
...
export default function EmployeesPage() {
  ...
  const [editForm, setEditForm] = useState({ name: "", email: "", base_salary: "", social_security: false, religion: "" });
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((e) => String(e.employee_id || "").toLowerCase().includes(q) || String(e.name || "").toLowerCase().includes(q) || String(e.email || "").toLowerCase().includes(q));
  }, [employees, search]);
  ...
  const openEdit = () => {
    ...
    setEditForm({ name: selectedEmployee.name || "", email: selectedEmployee.email || "", base_salary: selectedEmployee.base_salary || "", social_security: !!selectedEmployee.social_security, religion: selectedEmployee.religion || "" });
    setShowEdit(true);
  };
    ...
    <div className="space-y-6">
          ...
          {!selectedEmployee ? <div className="text-sm text-slate-400">{t("selectEmployee")}</div> : (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <div className="text-xs text-slate-500 mb-1">{lang === "ar" ? "الإجازات السنوية المتبقية" : "Annual leave remaining"}</div>
                  <span className={clsx("badge", Number(selectedBalance?.annual_remaining ?? 14) > 5 ? "badge-green" : Number(selectedBalance?.annual_remaining ?? 14) > 0 ? "badge-yellow" : "badge-red")}><Calendar size={11} />{selectedBalance?.annual_remaining ?? 14}</span>
                </div>
              </div>
            </div>
          )}
      ...
      {showAdd && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setShowAdd(false); }}>
          <div className="modal">
            <div className="modal-header"><h3 className="modal-title">{t("newEmployee")}</h3><button className="modal-close" onClick={() => setShowAdd(false)}><X size={18} /></button></div>
            <form onSubmit={handleAdd} className="space-y-4">
              {[["employee_id", t("employeeId"), "EMP-001", "text"], ["name", t("name"), t("name"), "text"], ["email", emailTitle, "employee@company.com", "email"], ["base_salary", t("baseSalary"), "0.00", "number"]].map(([key, label, ph, type]) => (
                <div key={key}><label className="form-label">{label} *</label><input type={type as string} className="form-input" value={(form as any)[key]} onChange={(e) => setForm((f) => ({ ...f, [key]: e.target.value }))} placeholder={ph as string} /></div>
              ))}
              ...
              <div className="flex items-center justify-between"><div><div className="text-sm font-medium text-slate-700">{t("socialSecurity")}</div></div><label className="toggle"><input type="checkbox" checked={form.social_security} onChange={(e) => setForm((f) => ({ ...f, social_security: e.target.checked }))} /><span className="toggle-slider" /></label></div>
      ...
      {showEdit && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setShowEdit(false); }}>
          <div className="modal">
            <div className="modal-header"><h3 className="modal-title">{lang === "ar" ? "تعديل الموظف" : "Edit Employee"}</h3><button className="modal-close" onClick={() => setShowEdit(false)}><X size={18} /></button></div>
            <form onSubmit={handleEdit} className="space-y-4">
              ...
              {[["name", t("name"), "text"], ["email", emailTitle, "email"], ["base_salary", t("baseSalary"), "number"]].map(([key, label, type]) => (
                <div key={key}><label className="form-label">{label} *</label><input type={type as string} className="form-input" value={(editForm as any)[key]} onChange={(e) => setEditForm((f) => ({ ...f, [key]: e.target.value }))} /></div>
              ))}
              ...
              <div className="flex items-center justify-between"><div><div className="text-sm font-medium text-slate-700">{t("socialSecurity")}</div></div><label className="toggle"><input type="checkbox" checked={editForm.social_security} onChange={(e) => setEditForm((f) => ({ ...f, social_security: e.target.checked }))} /><span className="toggle-slider" /></label></div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 rounded-lg bg-slate-50 border border-slate-200 p-3 text-sm"><div className="font-semibold text-slate-900">{selectedEmployee.name}</div><div className="text-slate-500 font-mono text-xs mt-1">{selectedEmployee.employee_id}</div></div>
              </div>
            </div>
