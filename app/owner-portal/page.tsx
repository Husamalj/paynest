    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-slate-200 px-4 h-16 flex items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-white font-black text-xs">CO</div>
        ...
        <div className="relative">
          <button onClick={() => setProfileOpen((o) => !o)} className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-100 transition-colors">
            ...
            <div className="hidden sm:block text-left min-w-0">
              ...
              <div className="text-[11px] text-brand-600 uppercase font-bold">{ar ? "المدير" : "OWNER"}</div>
            </div>
          ...
          {profileOpen && (
            <div className={clsx("absolute top-full mt-2 w-56 bg-white rounded-xl shadow-elevated border border-slate-200 overflow-hidden z-50", isRTL ? "left-0" : "right-0")}>
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
                ...
                <div className="mt-1.5 inline-flex px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 text-[10px] font-bold uppercase">{ar ? "المدير" : "OWNER"}</div>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                ...
                { label: ar ? "تكلفة الضمان" : "Social security cost", value: formatCurrency(fin.totalSS), icon: Shield, color: "text-brand-600 bg-brand-50" },
            ...
            <div>
              {/* Cost per department */}
              <div className="card">
                <div className="card-header"><div className="card-title"><Briefcase size={16} className="text-brand-600" />{ar ? "تكلفة كل قسم" : "Cost per department"}</div></div>
                    ...
                    {fin.deptList.map((d, i) => {
                      ...
                      return (
                        <div key={i}>
                          ...
                          <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden"><div className="h-full bg-brand-500 rounded-full" style={{ width: `${pct}%` }} /></div>
                        </div>
                      );
                    })}
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="card lg:col-span-2">
              <div className="card-header">
                <div className="card-title"><Users size={16} className="text-brand-600" />{ar ? "قائمة الموظفين" : "Employees"}</div>
                <button className="btn btn-primary btn-sm" style={{ background: "#006fc6" }} onClick={() => { setShowAddEmp(true); setAddEmpStep(1); setEmpForm({ ...emptyEmp }); setDocFiles({}); setError(""); }}>
                ...
                {employees.length === 0
                  ...
                  : (
                    <table>
                      ...
                      <tbody>
                        {employees.map((emp) => (
                          <tr key={emp.employee_id} className={clsx("cursor-pointer", emp.employee_id === selectedEmpId && "bg-brand-50")} onClick={() => setSelectedEmpId(emp.employee_id)}>
                            <td className="pl-3" />
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      {selectedEmp.photo_url ? (
                        <img src={selectedEmp.photo_url} alt={selectedEmp.name} onClick={() => setLightbox(selectedEmp.photo_url)} className="w-12 h-12 rounded-full object-cover cursor-pointer ring-2 ring-brand-100 flex-shrink-0" />
                      ) : (
        ...
        {tab === "hr_team" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="card lg:col-span-2">
              <div className="card-header">
                <div className="card-title"><ShieldCheck size={16} className="text-brand-600" />{ar ? "مديرو الموارد البشرية" : "HR Managers"}</div>
                <button className="btn btn-primary btn-sm" style={{ background: "#006fc6" }} onClick={() => { setShowAddHR(true); setHrForm({ ...emptyHR }); setError(""); }}>
              ...
              {hrs.length === 0
                ? (
                  <div className="text-center py-16 text-sm text-slate-400">
                    ...
                    <div className="mt-2"><button className="btn btn-primary btn-sm" style={{ background: "#006fc6" }} onClick={() => { setShowAddHR(true); setHrForm({ ...emptyHR }); }}><Plus size={13} />{ar ? "إضافة الأول" : "Add first"}</button></div>
                  </div>
                ) : (
                  <div className="table-wrapper">
                    <table>
                      ...
                      <tbody>
                        {hrs.map((hr) => (
                          <tr key={hr.id} className={clsx("cursor-pointer", hr.id === selectedHrId && "bg-brand-50")} onClick={() => setSelectedHrId(hr.id)}>
                            <td>
                              <div className="flex items-center gap-2">
                                <div className="w-7 h-7 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-xs font-bold">{hr.name?.[0]?.toUpperCase() || "H"}</div>
            ...
            <div className="card compact">
              ...
              {!selectedHR
                ...
                : (
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-xl font-bold flex-shrink-0">{selectedHR.name?.[0]?.toUpperCase() || "H"}</div>
                      <div className="min-w-0">
                        <div className="font-bold text-slate-900 truncate">{selectedHR.name}</div>
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 text-[10px] font-bold uppercase mt-0.5"><ShieldCheck size={10} />HR</span>
                      </div>
                    </div>
                    <div className="divide-y divide-slate-100">
        ...
        {tab === "team" && (
          <div className="card">
            <div className="card-header">
              <div className="card-title"><Users size={16} className="text-brand-600" />{ar ? "فريقي — الموظفون تحت إشرافي" : "My Team — employees I supervise"}</div>
              <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {subordinates.map((s) => (
                  <div key={s.id} className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 bg-slate-50">
                    <div className="w-9 h-9 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-sm font-bold flex-shrink-0">{(s.name || "?").charAt(0).toUpperCase()}</div>
            ...
            {announcements.length === 0 ? <div className="text-center py-12 text-sm text-slate-400">{ar ? "لا يوجد إعلانات" : "No announcements"}</div> : (
              <div className="space-y-3">
                {announcements.map((ann) => (
                  <div key={ann.id} className="p-4 bg-brand-50 border border-brand-100 rounded-xl">
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) { setShowAddEmp(false); setAddEmpStep(1); } }}>
          <div className="modal">
            ...
            <div className="flex items-center gap-3 pb-4">
              <div className={clsx("flex items-center gap-1.5 text-sm font-semibold", addEmpStep === 1 ? "text-brand-600" : "text-emerald-600")}>
                <div className={clsx("w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold", addEmpStep === 1 ? "bg-brand-600 text-white" : "bg-emerald-100 text-emerald-600")}>{addEmpStep > 1 ? <CheckCircle size={14} /> : "1"}</div>
                {ar ? "المعلومات" : "Info"}
              </div>
              <ChevronRight size={14} className="text-slate-300 flex-shrink-0" />
              <div className={clsx("flex items-center gap-1.5 text-sm font-semibold", addEmpStep === 2 ? "text-brand-600" : "text-slate-400")}>
                <div className={clsx("w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold", addEmpStep === 2 ? "bg-brand-600 text-white" : "bg-slate-100 text-slate-400")}>2</div>
            ...
            {addEmpStep === 2 && (
              <div className="space-y-4">
                ...
                <div className="flex justify-between gap-2 pt-2">
                  ...
                  <button className="btn btn-primary" style={{ background: "#006fc6" }} disabled={docUploading} onClick={handleUploadDocs}>
          ...
          <div className="modal">
            ...
            <form onSubmit={handleAddHR} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={addingHR} className="btn btn-primary flex-1" style={{ background: "#006fc6" }}>{addingHR ? (ar ? "جاري الإضافة..." : "Adding...") : (ar ? "إضافة" : "Add")}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="modal">
            ...
            <form onSubmit={handleEditHR} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={savingHR} className="btn btn-primary flex-1" style={{ background: "#006fc6" }}>{savingHR ? (ar ? "جاري الحفظ..." : "Saving...") : (ar ? "حفظ" : "Save")}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" dir={isRTL ? "rtl" : "ltr"} onClick={(e) => { if (e.target === e.currentTarget) setTaskTarget(null); }}>
          <form onSubmit={submitOwnerTask} className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <span className="font-bold text-slate-900 flex items-center gap-2"><CheckSquare size={18} className="text-brand-600" />{ar ? "تعيين مهمة" : "Assign Task"}</span>
              <button type="button" onClick={() => setTaskTarget(null)} className="text-slate-400 hover:text-slate-700"><X size={18} /></button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div className="flex items-center gap-2 text-sm text-slate-600"><div className="w-8 h-8 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center font-bold">{(taskTarget.name || "?").charAt(0).toUpperCase()}</div>{taskTarget.name}</div>
