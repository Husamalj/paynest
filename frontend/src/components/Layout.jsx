...
export default function Layout({ children, settings }) {
  ...
  const [expandedKeys, setExpandedKeys] = useState({ employees: true });
      ...
      >
        <div className="px-5 py-5 border-b border-slate-100">
          ...
          <div className="mt-4 rounded-xl bg-slate-50 border border-slate-200 p-3">
            ...
            <div className="mt-2 inline-flex px-2 py-1 rounded-full bg-brand-50 text-brand-700 text-[10px] font-bold uppercase">
              {role}
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto">
          <div className="text-[11px] font-semibold uppercase text-slate-400 tracking-wider px-3 mb-2">
            {t('overview')}
          </div>
                      ...
                      <div className="mt-1 pl-8 space-y-1">
                        {item.children.map((ch) => (
                          <NavLink
                            key={ch.key}
                            to={ch.path}
                            end={ch.path === '/employees'}
                            onClick={() => setSidebarOpen(false)}
                            className={({ isActive: isChActive }) =>
                              clsx(
            ...
            <form onSubmit={changePassword} className="space-y-4">
              <div>
                <label className="form-label">
                  {lang === 'ar' ? 'كلمة السر الحالية' : 'Current Password'}
                </label>
                <input
                  type="password"
              ...
              <div>
                <label className="form-label">
                  {lang === 'ar' ? 'تأكيد كلمة السر الجديدة' : 'Confirm New Password'}
                </label>
                <input
                  type="password"
                  className="form-input"
