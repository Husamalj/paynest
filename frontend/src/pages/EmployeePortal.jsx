...
export default function EmployeePortal() {
  ...
  const myTasks = useMemo(
    () => tasks.filter((task) => task.employee_id === employeeId),
    [tasks, employeeId]
  );

  const myLeaves = useMemo(
    () => leaves.filter((leave) => leave.employee_id === employeeId),
    [leaves, employeeId]
  );

  const myBalance = useMemo(
    () => balances.find((balance) => balance.employee_id === employeeId) || null,
    [balances, employeeId]
  );
      ...
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900">
                {employee?.name || savedUser.name || text.title}
              </h1>

              <p className="text-sm text-slate-500">
                {savedUser.email || text.subtitle}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {!isEmployeeLogin && (
              <select
                className="form-select min-w-0 sm:w-72"
                value={employeeId}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="card">
                ...
                <div className="text-xl font-bold text-slate-900">
                  {formatCurrency(myPayroll?.base_salary || employee.base_salary)}
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div className="text-xl font-bold text-emerald-700">
                  {formatCurrency(myPayroll?.net_salary)}
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div className="text-xl font-bold text-rose-700">
                  {formatCurrency(
                    Math.max(0, -(parseFloat(myPayroll?.adjustment) || 0)) +
                    (parseFloat(myPayroll?.deduction_total) || 0) +
                    (parseFloat(myPayroll?.social_security_deduct) || 0)
                  )}
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div
                  className={clsx(
                    'text-xl font-bold',
                    parseFloat(myPayroll?.hour_diff || 0) < 0
                      ? 'text-rose-700'
                      : 'text-emerald-700'
                  )}
                >
                  {(parseFloat(myPayroll?.hour_diff) || 0).toFixed(2)}
                </div>
              </div>
            </div>
              ...
              <div className="card">
                <div className="card-header">
                  <div className="card-title">
                    <Palmtree size={16} className="text-brand-600" />
                    {text.requestLeave}
                  </div>
                </div>
                ...
                <form className="space-y-4" onSubmit={submitLeave}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="form-label">{text.startDate}</label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="form-label">{text.endDate}</label>
                      <input
                        type="date"
                        className="form-input"
                  ...
                  {myBalance && (
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
                        <div className="text-xs text-slate-500">{text.annual}</div>
                        <div className="font-bold text-slate-900">
                          {myBalance.annual_remaining}
                        </div>
                      </div>

                      <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
                        <div className="text-xs text-slate-500">{text.sick}</div>
                        <div className="font-bold text-slate-900">
                          {myBalance.sick_remaining}
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <button className="btn btn-primary w-full" disabled={saving}>
                    {saving ? <span className="spinner" /> : <Send size={15} />}
                    {text.send}
                  </button>
                </form>
              </div>
                  ...
                  {myTasks.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {myTasks.map((task) => (
                        <div
                          key={task.id}
                          className="rounded-lg border border-slate-200 bg-white p-3 flex flex-wrap items-center justify-between gap-3"
                        >
                          <div>
                  ...
                  {announcements.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {announcements.slice(0, 4).map((item) => (
                        <div
                          key={item.id}
                          className="rounded-lg bg-brand-50 border border-brand-100 p-3"
                        >
                          <div className="font-semibold text-slate-900">
                            {item.title}
                          </div>
                    ...
                    <tbody>
                      {myLeaves.map((leave) => (
                        <tr key={leave.id}>
                          ...
                          <td>{statusBadge(leave.status, isRTL)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
