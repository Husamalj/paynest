...
const TABS = ['pending', 'active', 'suspended', 'all'];
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (createRes.data.pending) {
        ...
        const created = (allRes.data || []).find(c => c.slug === form.slug);
                      ...
                      <tr key={c.id}>
                        ...
                        <td>
                          <span className="flex items-center gap-1 text-slate-600">
                            <Users size={13} /> {c.employee_count ?? '—'}
                          </span>
                        </td>
                        <td>
                          ...
                          <div className="flex justify-end gap-2">
                            {isPending && <>
                              <button
                                ...
                                onClick={() => action(c.id, 'approve', `"${c.name}" approved`)}
                              >
                              ...
                              <button
                                ...
                                disabled={busyId === c.id}
                                onClick={() => action(c.id, 'reject', `"${c.name}" rejected`)}
                              >
                                <Ban size={13} /> Reject
                              </button>
                            </>}
                            {isActive && <>
                              <button
                                ...
                                disabled={busyId === c.id}
                                onClick={() => action(c.id, 'toggle-status', `"${c.name}" suspended`)}
                              >
                            ...
                            {isSuspended && <>
                              <button
                                className="btn btn-sm btn-success"
                                disabled={busyId === c.id}
                                onClick={() => action(c.id, 'approve', `"${c.name}" reactivated`)}
                              >
          ...
          <div className="modal">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <input className="form-input" value={form.companyName} onChange={e => autoSlug(e.target.value)} placeholder="Alpha Tech" required />
              </div>
              <div>
                <label className="form-label">Slug *</label>
                <input className="form-input" value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="alpha-tech" required />
              </div>
              <div>
                <label className="form-label">Owner Name *</label>
                <input className="form-input" value={form.ownerName} onChange={e => setForm(f => ({ ...f, ownerName: e.target.value }))} placeholder="Owner Name" required />
              </div>
              <div>
                <label className="form-label">Owner Email *</label>
                <input type="email" className="form-input" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="owner@company.com" required />
              </div>
              <div>
